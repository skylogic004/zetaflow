...
Time:
	...
	02-05-07 4.5
	03-05-07 5
	15-05-07 3h Can now charge bullets. Damage bonus follows a cosine curve (concave down, increasing)
	22-05-07 5h Changed way of storing levels and ai code. Editor matches it as well.
	24-05-07 3h
	25-05-07 2h Misc. Changed charge to X button.
	28-05-07 3h Making automated snapping for editor (picks closest part as parent)
	07-06-07 3h
	09-06-07 2h Can now go back and forth between edtior and game
	15-06-07 2h
	17-06-07 1h
	20-06-07 3h Save levels as strings, reload strings as levels. level stored as objects with arrays.
	21-06-07 3h Worked on php file to save data to mysql. Created zetaFlow database.
	22-06-07 2h zetaFlow.users and levels are timestamped when they are created.
	27-06-07 2.5h can load levels from gallery. key assignments changed. rotate part bug fixed. misc changes. getLevel() and onLevelComplete() created.
	28-06-07 5h new menu buttons, misc
	29-06-07 2h damage indicators 2h editor fixes
	30-06-07 2h bug fixes. plays and wins counter created
	01-07-07 2h fixing naming lexicon. added lives setting for user.
	02-07-07 4h sounds
	03-07-07 4h
	04-07-07 6.5h
	10-08-07 3h made zf.skylogic.ca website
	11-08-07 6h worked on website
	12-08-07 4h php files send email error reports
	13-08-07 3h
	14-08-07 4h
	18-08-07 3h
	29-08-07 5h launching site, maintence. upgraded to level 2 account, bought 15 gb bandwidth this month. total=~$15
	30-08-07 4h
	31-08-07 2h Rating system launched (gallery shows stars, game gives option to rate, php uses cookie to prevent multiple votes)
				Can open editor.tutorial from tools button
				Pause menu w/ rating window come up after completing/dieing in a custom level
	03-08-07 Removed <script type="text/javascript" src="fix_ieactivate.js"></script> from main.html. JS took a fair bit of bandwidth
	01-09-07 Added line 2,3 to ratingWindow.rating displays "0/100" when appropriate (it was displaying "/100" before)
	08-09-07 2h Getting login and gallery working. Calling getLevel from gallery isn't working yet
	08-09-07 3h Loging and gallery works.
	09-09-07 4h fixed php problems, error handler works properly
	15-09-07 3h fixed issues. delete, rename, edit works. login is persistant and can be saved to sharedObject
			 3h
	24-09-07 1h fixed register button (broke it in last update). uploading new version
	26-09-07 1h rearranged how core is set in the editor
	29-09-07 3h Core is set if "me" depth is 0. this should eliminate the multi core issue.
			 Edited removeWhitespace to allow it to remove empty arrays (for if removeWhitespace makes the array's only element be an empty string)
	30-09-07 3h Completely changed detectKeys. can now aim manually using mouse or keyboard. shooting is now key or click (not dependent on movement scheme)
	02-10-07 1h Can no longer edit someone else's level. Must login first. (The game still loads all the data, so it is by no means secure, but that one single IF statement should prevent noobs from stealing levels)
	20-10-07 2h Victory in game gives you the option to go to the gallery or the editor. Pause menu brings up rating window always.
	31-10-07 1h Circle thingy bullet (the vector version), gun and bullet made and coded - works flawlessly (which means my engine does what it should :D )
	Current total 176.25 hours
	04-11-07 6h New bullet tweaked (and named Singularity). Editor grouping functionality added
	07-11-07 2h Singularity renamed Singu in code only (trying to be bandwidth aware, although, really parts should be numbers if i were to be truly conserving).
				Added niceName option for each part. If a part has this var, it will be shown to user instead of link var
				Added some subtle animation to single and singu. Will add more animation cause it looks really really nice!
				Misc Fixes (control-d=duplicate, play button in editor, gun delay tweaks)
	16-11-07 2h More animations
	24-11-07 1h Animations pause and resume on pause
	08-12-07 1h Fixed glitch with AI window, wasn't being attached if it didn't exist (made editor_toggleAIwindow, editor_toggleLoginWin, and editor_toggleSaveAsWin)
	21-12-07 2h
	23-12-07 4h
	24-12-07 6h Uploaded update today. ToDo:4x "Grouping":Completed. 
	30-12-07 6h Worked on linking, should work soon. 
				Hidden/Public flag - might still need a 'complete' flag to ensure an accurate date
				ToDo:2x More categories:Completed
	01-01-08 8h worked on gallery.php and search.php
				lives counter: switches to number display when greater than 5
				added arena border
				"When the core is destroyed, make the invulnerablity of my gun last longer" - Check
	02-01-08 1h Uploaded update today. ToDo:4x Text editor:Completed.
	31-01-08 2h Working on colours (animated hit effect)
	10-02-08 Fixed hit sound. Hasn't worked ever since preloader was implemented :O
	16-02-08 3h Colors now entirely controlled by code. Parts color should be: Outline=0x666666, Fill=20% 0x666666
			 2h Colour picker. Still needs grayscale buttons
	23-02-08 2h Colour stuff. grayscale buttons
	02-03-08 1h Mute button
	30-04-08 4h AI windows and triggers's code
	05-05-08 6h
	31-05-08 7h triggers, front page feature level boxes
	07-05-08 5h Website updates
	15-06-08 6h Time trigger, first official contest planning
				-5x "Triggers" (ToDo item completed)
	18-06-08 4h
	20-06-08 2h Contest prep, autosave feature
	21-06-08 2h Contest prep
	24-06-08 5h Stop repeating triggers, error notification on infinitely repeating trigger
	03-09-08 4h Setting up click away ads
	2008-12-04 Fixed glitch where part count would increment by string "1" instead of integer 1 on level load
	2009-03-16 3h Changed level title textbox to NOT embed the font for any characters, thus supporting multiple languages from Unicode. Also fixed PHP files to connect to MySQL in UTF-8
	2009-04-22 8h CodeIgniter. Todo item no longer necessary: -MySQL queries should be this style: 		$query = sprintf("INSERT INTO stats_plays (ID, username, name) values (\"%s\", \"%s\", \"%s\")", mysql_real_escape_string($ID), mysql_real_escape_string($username), mysql_real_escape_string($name));
	2009-04-23 8h CodeIgniter
	2009-04-25 7h User Profile page in CI, statistics tables
	2009-04-27 2h
	2009-04-29 3h
	2009-05-06 5h
	2009-05-07 8h Fixed bug where destroying the core (by way of triggers, ai code, etc) immediately on frame 0 would not exit the game, because the gameLoop had yet to be initialized which happens AFTER the initial ai code. so i've stopped it from overriding the onEnterFrame with an IF statement
	2009-05-12 8h
	2009-05-13 10h
	2009-05-15 5h
	2009-05-16 6h
	2009-05-20 3h Renamed Account to Home. Created Account Settings page to save email, password, newsletter preference, language (there for show only atm)
	2009-06-09 3h making createLevel() lib function in php
	           3h highscores (top lives, top time)
	2009-06-10 10h Now completely XHTML Strict validated (except play.php, which holds the game SWF - it's still half HTML 4.01 transitional). Works in all browsers.
				   Switched saveLevel.php to CI system (/backend/saveLevel/)
	2009-06-11 3h AI_makeInvincible no longer sets HP to 9999999 (invincible flag is enough). AI_makeVulnerable no longer resets HP back to the part's starting HP. And neither of these automatically change the colour of parts anymore (i expect users to take care of colouring)
	2009-06-12 3h No more logging in via flash. PHP only. Made appropriate error/success messages in flash to handle the new conditions
			   2h getLevelsList converted to CI. Now uses the username stored in session, rather that POSTing which username to query. Still returns data in XML format.
			  
	2009-07-03 2h In timeTriggerExecute(), where addToCoreAIFunctions() was called, it was suppose to not do it if the AI function returned "initOnly", but it was using 'continue' instead of 'return' (in this case it's not in a for loop). Fixed that.
				  Also added a condition to AI_rotatePart() such that it's an initOnly if the rotate speed == 0
				  
				  Created setWidth, setHeight, setDistance, setHP, setGunDelay
	2009-07-14 3h Fixed bug: In editor, now calling editor_createCoreAIwindow() at the start. This forces the window's text field to be filled with the AI code that has "AI_" stripped from it. Without this, it worked most of time except when opening the Text edit or Display output  windows (which called editor_make_AI_code()) prior to initializing the AI code window
	2009-12-29 3h Added support for moderators. Can now delete comments and levels via GUI button. All mod activity recorded in modLog table.
	
	2010-12-30 3h
		Things to do when release update:
			-Post levels.as, allowing people to see initial HP values
			-Tip: makeTrigger,time,1,setHP,8000,6,7,8
			      (i dont remember why you may not want to use setHP by itself...but there was a good reason i'm sure...)
			-Tip: enable advanced view (text edit > advanced, then cancel text edit). This way mousing over parts (on the screen, not on the parts panel) will show their internal names. Also, gun parts will display timing information (frame # where they will fire, etc)
			-Tip: Take full control of weapon firing by disabling guns at the start: disableGun,partNumbers
	2011-01-18 2h Hammerhead Harrier video
	2011-01-19 3h Hammerhead Harrier video released, released update, fixed AI Code window not reappearing as it should (need global declaration of var blahWindow).
	2017-11-00 Created a "screenshot mode" where game is loaded but frozen on a specified frame (for backend use only)
	           Created "permanentlyInvincible" flag for ship for the screenshot mode.
ToDo:
	-Do windows _visible=false or removeMovieClip? may be better to remove.
	-Found out that variable set in GET and/or POST in php will be set as php variables. no need for $_GET['name']. perhaps should update previous files.
	-Player profiles? Earn credit/XP from winning levels (would motivate players=get players to end of level=show more ads=more $)
	-Make campaign only version (new levels would be nice) and distribute it to various portals.
	-3x "really wish players could give designers more feedback"
	-"Would it be possible to combine expert and keyboard aiming, for instance? If I could rotate the aiming point freely by moving without firing, but also have the key commands to change aim in tight spots, that would be the best of both worlds. Or else add commands to rotate the aim clockwise and counterclockwise."
	-2x "Would it be possible to allow opening other people's levels"
	-"Part list (for selecting parts that can't be clicked)"
	
	-"Shortcut keys for making parts bigger/smaller"
	-2x "Weapon Customisation"
	-2x "New weapons and parts would also be cool"
	-1x Line of sight rotating (rotate to ship if within scope)
	-DeleteLevel.php should delete via ID, not username/name
	-4x Completed flags, must be beatable by creator? (http://zf.skylogic.ca/phpbb/viewtopic.php?p=196)
	-2x Unlockables: http://zf.skylogic.ca/phpbb/viewtopic.php?p=231#231
	-"Make the forcefields optionally invincible through scripting, not automatically as they are now"
	-"Improve my auto fire accuracy over long distances"
	-1x Custom colors (parts, missiles)?
	-Right angle triangle
	-gallery.php: function genURL doesnt like getting less than 4 or null params. PHP does NOT support overloading functions.
	-gallery.php: php doesn't like undefined variables. namely the GET variables, how to work around?
	-Make use of php's trim() function to strip whitespace from usernames
	-Registration: Flash considers a group of korean letters as 1 character, whereas php does it correctly. Dont think there's anything i can do about that.
	-When a login fails "Something didnt work :(", the login button no longer works?
	-Clickable pause button
	-Ship editor (to be used as player's ship). Level maker chooses which ship(s) the players can choose from.
	-levels table should have UID not username, gallery should link to user ID not username
	-2009-05-13 LID: 4742. shockingly high stats. Changing values, old values: 85.7892 rating, 1874 votes, 10194 plays, 3028 wins.
			new values: 100 votes. check stats at a later date
			2009-05-20: now 85.6687 rating, 118 votes, 10282 plays, 3052 wins
			new values: 10 votes
	-paginate /user/search. and also profile pages if i want
	-BUG - Pressing "select all" scales the pieces (x only? is it adapting to the core?)
	-BUG - Quality: setting to 'low' is not remembered. when the level restarts it goes back to high --this seems to be working in the latest version
	-BUG - "it only happens if you get hit at the exact moment that your shield runs out. The shield is actually gone, but its image reappears, making you a much bigger target until the image vanishes again." (swartzer)
	
	-AJAX to determine if user logged in, and reload page if necessary
	-adwords promotional code: ABHH-XRHL-Z9TQ-E5AR-TQ
	-AdWords info
	-Lives: par system, have set goal of lives rather than a limit. http://zetaflow.skylogic.ca/forum/viewtopic.php?p=1217&sid=4f6090548f4830b35cdb3833f65ea4e5#p1217
Shoot 'Em Up Flash Game 
Thousands of user created levels 
Free online flash shmup 
zetaflow.skylogic.ca
...
Forum database: forum_zetaflow
Usr: admin_zetaflow
PW: zfmd4000000
Forum admin: MD, mb.....

grant select,insert,update,delete on zetaFlow.* to limited@"localhost" identified by 'zfmd4000000';
grant select,insert,update,delete on zetaFlow.* to zflimited@"localhost" identified by 'zfmd4000000';

create table users (username varchar(20), pw varchar(32));
create table levels (username varchar(20), level text, name varchar(255), description varchar(255), rating float unsigned, plays int unsigned, wins int unsigned);

//crossdomain.xml in root folder (subdomain) //Hosted by: http://games.mochiads.com/
<?xml version="1.0"?>
...
setcookie(name, value, expire |*time()+seconds*|, path |*can skip to set as current*|, ".skylogic.ca");

<EMBED src="http://zf.skylogic.ca/phpbb/images/singularity.swf" loop=false menu=false quality=high bgcolor=#000000  WIDTH="200" HEIGHT="200" NAME="Singularity" ALIGN="" TYPE="application/x-shockwave-flash" PLUGINSPAGE="http://www.macromedia.com/go/getflashplayer"></EMBED>
[url=http://zf.skylogic.ca/phpbb/images/singularity.swf]Click[/url]
 
LINKS:
	http://www.kotaro269.com/archives/50409381.html
	http://www.jesper.nu/forum/onlinespel/32965-kommentera
	http://www.omoshirogame.com/
	http://flashongame.blog11.fc2.com/blog-entry-864.html#comment_head
	http://www.flabber.nl/archief/021386.php?c=616092
	http://www.lazylaces.com/article.asp?p=2887#comments
	http://www.flashgames.de/index.php?onlinespiele=5142
	http://www.fazed.net/view/?id=14299&last
	http://www.poptronics.fr/Son-nom-est-Zeta-Zeta-Flow?var_recherche=zeta%20flow
	http://skmwin.net/archives/002124.html
	http://www.funcaptain.de/coole-games/zeta-flow
	http://www.pcwelt.de/start/gaming_fun/minispiel_des_tages/91796/index.html
	http://flash.plasticthinking.org/2007/08/zetaflow.html
	http://www.transbuddha.com/content/zeta-flow/
	*http://www.metafilter.com/64352/ZetaFlow
	http://timeforprocrastination.blogspot.com/2007/09/zeta-flow.html
	http://www.shmuptacular.com/News/p2_articleid/193
	http://www.123spill.no/actionindex.htm
	http://blog.wired.com/games/2007/09/browser-game-ze.html
	
	http://flashgametoday.jp/2008/10/21050000.php
	http://wearegame.blogspot.com/2007/08/zeta-flow.html

ValueClick Media:
Usr: skylogic
Pw: mb...etc
Site: zetaflow.skylogic.ca
